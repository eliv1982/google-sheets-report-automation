...
def _run_write_and_format_operations(
    client: GoogleSheetsClient, temp_sheet_name: str
) -> None:
    """Exercise write/read/formatting calls against an already-created sheet.

    Raises GoogleSheetsError or RuntimeError on failure.
    """
    client.write_range(
...
def main() -> int:
    ...
    try:
        ...
        _run_write_and_format_operations(client, temp_sheet_name)
    except (GoogleSheetsError, RuntimeError) as error:
        operation_error = error
        print(f"Write smoke-test failed: {error}", file=sys.stderr)
